...
public class Level4Controller {
    ...
    private void initialize() {
        ...
        ExpectroPatronumButton.setOnAction(event -> optionChoise("Accio"));
    ...
    private void handleAttack(String attackType) {
        if (isPlayerTurn) {
            ...
            if (damage > 0) {
                ...
                specialOption();
                animateCharacter();
                hideAttackOptionsBox(); // 隐藏attackOptionsBox
                isPlayerTurn = false;
            }
        }
    }
    private void hideAttackOptionsBox() {
        attackOptionsBox.setVisible(false);
    }
    ...
    private void optionChoise(String spellchoise){
        if (spellchoise.equals("Accio")){
            System.out.println("You defeated him completely and he was run away!");
            enemyHealth -=20;
            setNextLevelOptionBox();

        }
        else {
            System.out.println("He was not afraid of you, you have been kill");
        }
    ...
    private void onStartButtonClick(ActionEvent event) throws IOException {
        Parent createParent = FXMLLoader.load(getClass().getResource("Level5.fxml"));
